...
<template>
  <v-card class="my-4" color="#f7f7f7" :loading="loading">
    <v-sheet
    ...
    <div class="d-flex align-center justify-between justify-space-between">
      <v-card-title class="py-1">
        {{ task.title.substring(1, 20) }}
      </v-card-title>

      <div class="pe-4">{{ task.status }}</div>
    </div>

    <v-card-subtitle>
      {{ task.description.substring(1, 20) }}
    </v-card-subtitle>
    ...
    <v-card-text class="pb-2 pt-2">
      ...
      <div>Completed at : {{ task.completed_at ?? 'Not completed' }}</div>
    </v-card-text>
